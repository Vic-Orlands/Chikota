<script lang="ts">
  ...
  let reminderBookmarks = $derived($bookmarks.filter((b) => b.reminderAt));
...
<Dialog.Root bind:open>
  ...
  <Dialog.Content class="sm:max-w-[500px]">
    <Dialog.Header>
      ...
      <Dialog.Description>manage your bookmark reminders</Dialog.Description>
    </Dialog.Header>
    ...
    <div class="space-y-4">
      ...
      {:else}
        {#each reminderBookmarks as bookmark (bookmark.id)}
          <div
            class="flex items-start gap-3 p-3 rounded-lg border border-border/50 bg-secondary/20"
          >
            <img

...
Generate the Xcode project with `xcodegen generate --spec macos/project.yml`, select the `Chikota` scheme, and run it on **My Mac**. The development build uses ad-hoc “Sign to Run Locally” signing and does not require an Apple Developer account. Create a connection code from the browser-extension sheet in the web app and paste it into the Mac app with the deployed chikota URL. Control-click the macOS desktop, choose **Edit Widgets**, search for **chikota**, and add the widget.
